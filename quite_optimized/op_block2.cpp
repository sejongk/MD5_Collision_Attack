...
//keep T and F for checking difference.
Word T[64] = { 0, };
Word F[64] = { 0, };
...
*/
void Func(int round, Word* a, Word b, Word c, Word d, Word x, Word s, Word ac, int t) {
	switch (round) {
	case 0: (*a) += F((b), (c), (d)) + (x)+(Word)(ac); break;
	case 1: (*a) += G((b), (c), (d)) + (x)+(Word)(ac); break;
	case 2: (*a) += H((b), (c), (d)) + (x)+(Word)(ac); break;
	...
	switch (round) {
	case 0: F[t] = F((b), (c), (d)); break;
	case 1: F[t] = G((b), (c), (d)); break;
	case 2: F[t] = H((b), (c), (d)); break;
	case 3: F[t] = I((b), (c), (d)); break;
	}
	T[t] = (*a);
...
random_device random;                                //하드웨어 리소스를 기반으로 난수를 생성
mt19937 engine(random());                            //메르센 트위스터 방식으로 난수를 생성하겠다는 선언
// uniform distribution
uniform_int_distribution<Word> distribution(0, (((unsigned long long)1 << 32) - 1));  //난수의 범위와 자료형 정의. rand() % 100과는 다르게 100도 나올 수 있음
...
void getQ(int t) {
	...
	Func(round, &newQ, P[3], P[2], P[1], M[W_idx[t - 1]], RC[t - 1], AC[t - 1], t-1);
 	Q[t] = newQ;
}
...
bool verify_T() { // T22[17]=0, T34[15]=0 for both block 1 and block 2
	...
	cout << "verify T result: " << ((((T22 >> 17) & 1) == 1) && (((T34 >> 15) & 1) == 1)) << "\n";
	print_Q(T22);
	print_Q(T34);
...
int main(void) {
	...
	*/
	...
	state[0] = 0xab2d10f0; // Q[-3]
	state[1] = 0xa3805223; // Q[0]
	state[2] = 0x9b6d1f1b; // Q[-1]
	state[3] = 0x97f67ec1; // Q[-2]
	...
	for (int i = 0; i <= 63; i++) {
		cout << "W diff " << i << " => ";
		print_Q((M[W_idx[i]] - tmp_W[i]));
	}

	cout << "F 차분 체크\n";
	for (int i = 0; i <= 63; i++) {
		cout << "F diff " << i << " => ";
		print_Q((Word)(F[i] - tmp_F[i]));
	}

	cout << "T 차분 체크\n";
	for (int i = 0; i <= 63; i++) {
		cout << "T diff " << i << " => ";
		print_Q((Word)(T[i] - tmp_T[i]));
		if (i == 22) {
			cout << "X' => "; print_Q(T[i]);
			cout << "X => ";  print_Q(tmp_T[i]);
		}
	}
